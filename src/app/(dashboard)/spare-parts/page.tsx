  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 capitalize">{cType.replace('_', ' ')}s Inventory</h1>
          <p className="text-slate-500">Track and manage individual {cType.replace('_', ' ')} serials</p>
        </div>
        {user?.role === 'admin' && (
          <Button leftIcon={<Plus className="h-4 w-4" />} onClick={() => setShowAdd(true)}>Add Spare Part</Button>
        )}
      </div>
      ...
      <Modal open={showAdd} onClose={() => { setShowAdd(false); reset() }} title={`Register New Spare Part`} subtitle={`Add a spare part to your company stock`} size="md">
